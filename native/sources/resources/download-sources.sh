...
if [[ $# -ne 3 ]]; then
  echo "usage: download-sources.sh MANIFEST RETRIES OUT" >&2
  exit 2
fi
...
if [[ ! -f "$manifest" ]]; then
  echo "source manifest not found: $manifest" >&2
  exit 2
fi

mkdir -p "$output_dir"
cp "$manifest" "$output_dir/sources.tsv"
...
download_verified() {
  ...
  for candidate in "${candidates[@]}"; do
    ...
    rm -f "$partial"
    echo "[sources] mirror failed: $candidate" >&2
  done

  echo "[sources] all mirrors failed: $(basename "$target")" >&2
  return 1
}
...
while IFS=$'\t' read -r checksum filename url; do
  ...
  if [[ -f "$target" ]] && printf '%s  %s\n' "$checksum" "$target" | md5sum --check --status; then
    echo "[sources] cache hit: $filename"
    continue
  fi

  echo "[sources] downloading: $filename"
...
cat >"$output_root/opt/xbee-lfs-native/sources-metadata.yaml" <<EOF
schema-version: 1
lfs-book: "13.0"
stages:
  - 5
